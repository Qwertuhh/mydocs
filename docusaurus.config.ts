import { themes as prismThemes } from "prism-react-renderer";
import type { Config } from "@docusaurus/types";
import type * as Preset from "@docusaurus/preset-classic";

// This runs in Node.js - Don't use client-side code here (browser APIs, JSX...)

const config: Config = {
  title: "MyDocs",
  tagline: "This docs is to help you get proper context for My Projects",
  favicon: "img/favicon.ico",

  // Future flags, see https://docusaurus.io/docs/api/docusaurus-config#future
  future: {
    v4: true, // Improve compatibility with the upcoming Docusaurus v4
  },

  // Set the production url of your site here
  url: "https://qwertuhh-mydocs.netlify.app",
  // Set the /<baseUrl>/ pathname under which your site is served
  // For GitHub pages deployment, it is often '/<projectName>/'
  baseUrl: "/",

  // GitHub pages deployment config.
  // If you aren't using GitHub pages, you don't need these.
  organizationName: "qwertuhh", // Usually your GitHub org/user name.
  projectName: "mydocs", // Usually your repo name.

  onBrokenLinks: "throw",

  // Even if you don't use internationalization, you can use this field to set
  // useful metadata like html lang. For example, if your site is Chinese, you
  // may want to replace "en" with "zh-Hans".
  i18n: {
    defaultLocale: "en",
    locales: ["en"],
  },

  presets: [
    [
      "classic",
      {
        docs: {
          sidebarPath: "./sidebars.ts",
          // Please change this to your repo.
          // Remove this to remove the "edit this page" links.
          editUrl: "https://github.com/qwertuhh/mydocs/tree/main/docs",
        },
        blog: {
          showReadingTime: true,
          feedOptions: {
            type: ["rss", "atom"],
            xslt: true,
          },
          // Please change this to your repo.
          // Remove this to remove the "edit this page" links.
          editUrl: "https://github.com/qwertuhh/mydocs/tree/main/docs",
          // Useful options to enforce blogging best practices
          onInlineTags: "warn",
          onInlineAuthors: "warn",
          onUntruncatedBlogPosts: "warn",
        },
        theme: {
          customCss: "./src/css/custom.css",
        },
      } satisfies Preset.Options,
    ],
  ],

  themes: ["@docusaurus/theme-mermaid"],
  markdown: {
    mermaid: true,
  },
  themeConfig: {
    // Replace with your project's social card
    image: "img/mydocs-social-page.png",
    colorMode: {
      respectPrefersColorScheme: true,
    },
    navbar: {
      title: "My Docs",
      logo: {
        alt: "Qwertuhh Logo",
        src: "img/neutral-qwertuhh-black-logo.svg",
        srcDark: "img/neutral-qwertuhh-white-logo.svg",
      },
      items: [
        {
          type: "docSidebar",
          sidebarId: "CompliancesSidebar",
          position: "left",
          label: "Compliances",
        },
        { to: "/blog", label: "Blog", position: "left" },
        {
          href: "https://github.com/qwertuhh/mydocs",
          label: "GitHub",
          position: "right",
        },
        {
          type: "dropdown",
          label: "Repository",
          position: "left",
          items: [
            {
              type: "doc",
              label: "MyBackend",
              docId: "Projects/MyBackend/overview", // ✅ valid docId
            },
            {
              type: "doc",
              label: "Portfolio",
              docId: "Projects/Portfolio/overview", // ✅ another valid docId
            },
          ],
        },
      ],
    },
    footer: {
      style: "dark",
      links: [
        {
          title: "Docs",
          items: [
            {
              label: "Projects",
              to: "/docs/projects/intro",
            },
          ],
        },
        {
          title: "Community",
          items: [
            {
              label: "Discord",
              href: "https://discordapp.com/invite/qwertuhh.dev",
            },
            {
              label: "X",
              href: "https://x.com/qwertuhh132407",
            },
          ],
        },
        {
          title: "More",
          items: [
            {
              label: "Blog",
              to: "/blog",
            },
            {
              label: "GitHub",
              href: "https://github.com/qwertuhh/mydocs",
            },
          ],
        },
      ],
      copyright: `Copyright © ${new Date().getFullYear()} Medha. Built by qwertuhh under the MIT License.`,
    },
    prism: {
      theme: prismThemes.github,
      darkTheme: prismThemes.dracula,
    },
  } satisfies Preset.ThemeConfig,
};

export default config;
